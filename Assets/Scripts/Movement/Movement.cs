...
{
    public float Terrain_entering_cost(Unit unit ,Hex.Hex_terrain terrain){
        if(unit is General || unit is Supply_wagon) return 1;
    ...
    public bool Is_edge_easily_passable(Edge.Edge_hex_connector connector){
        return (connector == Edge.Edge_hex_connector.ROAD || connector == Edge.Edge_hex_connector.RAILWAY);
    }

    public float Edge_passing_cost(Edge.Edge_separator separator){
    ...
    public bool Is_hex_enterable(Hex.Hex_terrain terrain){
        return (terrain != Hex.Hex_terrain.LAKE && terrain != Hex.Hex_terrain.SEA);
    }

    public bool Is_edge_passable(Edge edge){
    ...
    public (bool,Edge) Are_hexes_connected(Hex hex_src, Hex hex_dest){
        foreach(Hex.Edge_info edge in hex_src.Edges) {
            if(edge.Value.Neighbour_hex == hex_dest) return (true,edge.Value);
        }
        return (false,null);
    }
    public bool Movement_posibility(Hex hex_src, Hex hex_dest){
    ...
    public float Calculate_movement_cost(Unit unit, Hex hex_src, Hex hex_dest){
        (bool connection_possible, Edge edge) connection_info = Are_hexes_connected(hex_src,hex_dest);
        if(Is_edge_easily_passable(connection_info.edge.Connector)) return unit switch{
                Supply_wagon _ => 0.5f,
                _ => 1f,
            };
        else{
