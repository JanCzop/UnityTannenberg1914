...
public class Movement : MonoBehaviour
{
    public static float Terrain_entering_cost(Unit.Unit_type type ,Hex.Hex_terrain terrain){
        if(type == Unit.Unit_type.GENERAL ||
        type == Unit.Unit_type.SUPPLY_WAGON) return 1;
    ...
    public static bool Movement_posibility(Hex hex_src, Hex hex_dest){
        (bool connection_possible, Edge edge) = Are_hexes_connected(hex_src, hex_dest);
        if(!connection_possible) return false;
        if(!Is_hex_enterable(hex_dest.Terrain)) return false;
        if(!Is_edge_passable(edge)) return false;
        return true;
    }
    public static float Calculate_movement_cost(Unit.Unit_type type, Hex hex_src, Hex hex_dest){
        (bool connection_possible, Edge edge) = Are_hexes_connected(hex_src,hex_dest);
        if(Is_edge_easily_passable(edge.Connector)) return type switch{
                Unit.Unit_type.SUPPLY_WAGON => 0.5f,
                _ => 1f,
            };
        else{
            float cost = 1f;
            cost += Edge_passing_cost(edge.Separator);
