            ...
            if (hex_object != null){
                ...
                if (hex_component != null){
                    if(i != Hexmap.MAP_LENGTH-1 && i != Hexmap.MAP_LENGTH-2) 
                        hex_component.Edges.Add(new Hex.Edge_info(0, new Edge(map.Hexes[i+2][j].GetComponent<Hex>())));
                    if(i != Hexmap.MAP_LENGTH-1 && j != Hexmap.MAP_HEIGHT-1) {
                        if(i %2 == 0) 
                            hex_component.Edges.Add(new Hex.Edge_info(1, new Edge(map.Hexes[i+1][j+1].GetComponent<Hex>())));
                        else 
                            hex_component.Edges.Add(new Hex.Edge_info(1, new Edge(map.Hexes[i+1][j].GetComponent<Hex>())));
                    }
                    if(i != 0 && j != Hexmap.MAP_HEIGHT-1) {
                        if(i %2 == 0) 
                            hex_component.Edges.Add(new Hex.Edge_info(2, new Edge(map.Hexes[i-1][j+1].GetComponent<Hex>())));
                        else 
                            hex_component.Edges.Add(new Hex.Edge_info(2, new Edge(map.Hexes[i-1][j].GetComponent<Hex>())));
                    }
                    if(i != 0 && i != 1) 
                        hex_component.Edges.Add(new Hex.Edge_info(3, new Edge(map.Hexes[i-2][j].GetComponent<Hex>())));
                    if(i != 0 && j != 0) {
                        if(i %2 == 0) 
                            hex_component.Edges.Add(new Hex.Edge_info(4, new Edge(map.Hexes[i-1][j].GetComponent<Hex>())));
                        else 
                            hex_component.Edges.Add(new Hex.Edge_info(4, new Edge(map.Hexes[i-1][j-1].GetComponent<Hex>())));
                    }
                    if(i != Hexmap.MAP_LENGTH-1 && j != 0) {
                        if(i %2 == 0) 
                            hex_component.Edges.Add(new Hex.Edge_info(5, new Edge(map.Hexes[i+1][j].GetComponent<Hex>())));
                        else 
                            hex_component.Edges.Add(new Hex.Edge_info(5, new Edge(map.Hexes[i+1][j-1].GetComponent<Hex>())));
                    }
                }
